...
deploy_native(){
  install_deps
  install_singbox
  write_config || die "配置生成失败，未覆盖现有配置"
  info "检查配置 ..."
  ENABLE_DEPRECATED_WIREGUARD_OUTBOUND=true "$BIN_PATH" check -c "$CONF_JSON" || die "sing-box 配置校验失败"
  info "写入并启用 systemd 服务 ..."
  write_systemd || die "systemd 服务写入失败"
  systemctl restart "${SYSTEMD_SERVICE}" || die "sing-box 启动失败，请查看 journalctl -u ${SYSTEMD_SERVICE}"
  open_firewall
...
landing_apply(){
  ...
  validate_landing_input "$host" "$port" "$scope" || return 1
...
configure_landing(){
  ...
  validate_landing_input "$phost" "$pport" "$pscope" || {
    read -rp "回车返回..." _ || true
    return 0
  }
...
test_landing(){
  ...
  local ip
  ip="$(socks5_probe "$LANDING_HOST" "$LANDING_PORT" "$LANDING_USERNAME" "$LANDING_PASSWORD" || true)"
  if [[ -n "$ip" ]]; then
...
vpngate_state(){
  ...
  if systemctl is-active --quiet "$VPNGATE_SERVICE" 2>/dev/null && \
     systemctl is-active --quiet "$VPNGATE_SB_SERVICE" 2>/dev/null && \
     ip link show "$VPNGATE_DEV" >/dev/null 2>&1 && \
     ss -lnt 2>/dev/null | grep -Eq "127\\.0\\.0\\.1:${VPNGATE_SOCKS_PORT}[[:space:]]"; then
...
vpngate_fetch_servers(){
  ...
  local tmp="${VPNDIR}/servers.csv.tmp.$$"
  if ! curl -fsSL --retry 3 --connect-timeout 8 --max-time 30 -A 'Mozilla/5.0' \
    'https://www.vpngate.net/api/iphone/' > "$tmp"; then
    rm -f "$tmp"
    warn "VPN Gate 节点列表下载失败"
    return 1
  fi
  mv -f "$tmp" "$VPNDIR/servers.csv"
  [[ -s "$VPNDIR/servers.csv" ]] || { warn "VPN Gate 节点列表为空"; return 1; }
}

vpngate_candidate_rows(){
  local pattern="${1:-JP}"
  awk -F, -v p="$pattern" '
    BEGIN { OFS = "," }
    NR > 2 && substr($1, 1, 1) != "#" {
      ...
      needle = tolower(p)
      b64 = $NF
      if (b64 !~ /^[A-Za-z0-9+\/]+={0,2}$/) next
      if (index(country, needle) > 0 || code == toupper(p) ||
          index(tolower($1), needle) > 0 || index(tolower($2), needle) > 0) print
    }
  ' "$VPNDIR/servers.csv" | sort -t, -k3,3nr
}

vpngate_select_server(){
  local pattern="${1:-JP}" rank="${2:-1}" selected
  selected="$(vpngate_candidate_rows "$pattern" | sed -n "${rank}p")"
  if [[ -z "$selected" ]]; then
    warn "没有找到匹配“${pattern}”的 VPN Gate 节点"
    return 1
  fi

  VPNGATE_HOST="$(printf '%s' "$selected" | awk -F, '{print $1}')"
  VPNGATE_IP="$(printf '%s' "$selected" | awk -F, '{print $2}')"
  VPNGATE_SCORE="$(printf '%s' "$selected" | awk -F, '{print $3}')"
  VPNGATE_COUNTRY="$(printf '%s' "$selected" | awk -F, '{print $6}')"
  printf '%s\n' "$selected" | awk -F, '{print $NF}' > "$VPNDIR/server.b64"
...
vpngate_decode_and_sanitize(){
  ...
  grep -q '<ca>' "$raw" || { warn "OpenVPN 配置缺少 CA 证书"; return 1; }
...
ExecStopPost=/bin/bash $VPNDIR/route-down.sh
TimeoutStopSec=20
...
BindsTo=$VPNGATE_SERVICE

[Service]
Type=simple
User=$VPNGATE_USER
Group=$VPNGATE_USER
ExecStartPre=$BIN_PATH check -c $VPNDIR/sing-box.json
...
vpngate_stop_services(){
  systemctl disable --now "$VPNGATE_SB_SERVICE" >/dev/null 2>&1 || true
  systemctl disable --now "$VPNGATE_SERVICE" >/dev/null 2>&1 || true
  [[ -x "$VPNDIR/route-down.sh" ]] && "$VPNDIR/route-down.sh" >/dev/null 2>&1 || true
}
...
vpngate_start_selected(){
  ...
  if ! systemctl enable --now "$VPNGATE_SERVICE" >/dev/null 2>&1; then
    warn "OpenVPN 服务启动失败"
    return 1
  fi
  if ! vpngate_wait_tunnel; then
    warn "OpenVPN 隧道未在限定时间内就绪"
    journalctl -u "$VPNGATE_SERVICE" -n 40 --no-pager || true
    return 1
  fi

  if ! systemctl enable --now "$VPNGATE_SB_SERVICE" >/dev/null 2>&1; then
    warn "VPN Gate 本地 SOCKS5 服务启动失败"
    return 1
  fi
  local i
  for i in {1..20}; do
  ...
  warn "VPN Gate SOCKS5 出口测试失败"
  journalctl -u "$VPNGATE_SERVICE" -u "$VPNGATE_SB_SERVICE" -n 60 --no-pager || true
  return 1
}
...
configure_vpngate(){
  ...
  local pattern scope ip rank=1 max_rank=5 connected=0
  ...
  while (( rank <= max_rank )); do
    if ! vpngate_select_server "$pattern" "$rank"; then break; fi
    info "尝试第 ${rank} 个候选：${VPNGATE_COUNTRY} ${VPNGATE_HOST} (${VPNGATE_IP}) score=${VPNGATE_SCORE}"
    if vpngate_decode_and_sanitize && vpngate_start_selected; then
      ip="$VPNGATE_TEST_IP"
  ...
  if (( connected == 0 )); then
    ...
    VPNGATE_ENABLED=false
    save_vpngate_meta
    warn "前 ${max_rank} 个 VPN Gate 候选均不可用，已停止失效链路并恢复直连/WARP"
    read -rp "回车返回..." _ || true
    return 0
  fi
  read -rp "回车返回..." _ || true
