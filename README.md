...
菜单 `12` 会从 GitHub 下载临时文件，检查脚本标识、版本格式和 Bash 语法，并显示当前文件与下载文件的 SHA-256。只有输入大写 `UPDATE` 后才会替换当前管理脚本；替换前会在同目录生成 `.bak.YYYYMMDDHHMMSS` 备份，替换完成后自动重新加载面板。更新只替换脚本本身，不会删除 `/opt/sing-box` 下的配置、凭据、端口、域名规则或 VPN Gate 数据。
