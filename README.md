...
- 菜单内更新管理脚本：显示当前/远程版本和 SHA-256，输入 `UPDATE` 后才原子替换，并自动保留旧脚本备份。
- 失败安全：公网 IP 获取失败不生成伪分享链接；换端口、关闭落地和配置重启失败时恢复旧配置；systemd 启动前自动校验 sing-box 配置。
- systemd 基础隔离：主 sing-box 服务默认启用 `NoNewPrivileges`、临时目录隔离、只读系统目录和地址族限制，运行数据目录保持可写。
- 管理面板带 `flock` 互斥锁，避免两个终端同时改端口、域名规则或落地配置造成状态互相覆盖。
- 主配置使用临时文件生成并校验后替换，不会因为生成失败留下半截 JSON。
- 凭据、WARP 密钥、落地信息、端口和环境状态文件统一按 `600` 保存；分享链接和 `/opt/sing-box/creds.env` 不要公开粘贴。
...
- `12`：检查并更新管理脚本；配置、凭据、端口和服务数据不会被覆盖。

当前主菜单采用循环交互，连续执行管理操作不会不断嵌套 Bash 函数；任何需要返回的操作都回到同一个菜单。

也支持基础命令行入口，便于 SSH、监控和故障排查：
...
不带参数仍然打开交互面板；`check` 只校验当前配置，不重启服务；`update` 仍需明确输入 `UPDATE` 才会替换脚本。
...
菜单 `12` 会先读取 GitHub `main` 的最新提交 SHA，再按提交固定地址下载临时文件，检查脚本标识、版本格式、Bash 语法并显示当前文件与下载文件的 SHA-256，避免 Raw CDN 缓存导致旧版本被误判为最新版。只有输入大写 `UPDATE` 后才会替换当前管理脚本；替换前会在同目录生成 `.bak.YYYYMMDDHHMMSS` 备份，替换完成后自动重新加载面板。更新只替换脚本本身，不会删除 `/opt/sing-box` 下的配置、凭据、端口、域名规则或 VPN Gate 数据。
...
- 应用 `sysctl --system` 后会再次直接写入本次配置，减少发行版 `/etc/sysctl.conf` 后加载的旧值覆盖；如果云厂商内核强制较低上限，会显示实际值和“宿主机限制/覆盖”，不会把不可写入的目标值伪报为已生效。
- 核心验收以 BBR、当前网卡实际 fq、TCP Fast Open、MTU 探测等为主；TCP/UDP 缓冲区和 backlog 属于附加优化，若宿主机设有更低上限会单独提示，不会把核心线路状态误报为失败。
